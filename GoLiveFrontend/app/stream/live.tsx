...
const LiveStreamApp: React.FC = () =>  {
  ...
  useEffect(() => {
    ...
    const handleStreamUpdate = (data: any) => {
      ...
      if (data.type === 'stream_started') {
        // Auto-refresh when a new stream starts
        console.log('🔄 New stream started, refreshing...');
        fetchLiveStreams(true);
        ...
        // Refresh to remove ended stream from list
        setTimeout(() => {
          fetchLiveStreams(true);
        }, 2000); // Small delay to ensure backend has updated
      }
    };
    
    websocketService.addListener('stream_update', handleStreamUpdate);
    
    // Cleanup on unmount
    return () => {
      websocketService.removeListener('stream_update', handleStreamUpdate);
  ...
  const [videoLoadAttempts, setVideoLoadAttempts] = useState<{ [id: string]: number }>({});
  ...
  const renderStream = ({ item, index }: { item: any, index: number }) => {
    ...
    const followerCount = item.followerCount ? formatFollowers(item.followerCount) : "0";
    const isVerified = !!(streamerUser && "verified" in streamerUser && streamerUser.verified);
    const viewersCount = item.viewerCount ? formatFollowers(item.viewerCount) : "0";
    const videoUrl = item.muxPlaybackId ? `https://stream.mux.com/${item.muxPlaybackId}.m3u8` : null;
    // Test with a sample video if Mux stream fails
    const testVideoUrl = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4";
    const finalVideoUrl = videoUrl || testVideoUrl;
    ...
    console.log('🔍 Stream Debug:', {
      ...
      formattedViewers: viewersCount,
      formattedFollowers: followerCount
    });
    
    console.log('🎥 Video URL for stream', item.id, ':', videoUrl);
    ...
    return (
      <View style={[styles.streamContainer, { height: VISIBLE_HEIGHT, top: 0, left: 0, right: 0, bottom: 0 }]}> 
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          {!videoError[item.id] ? (
            <Video
              source={videoUrl ? { uri: videoUrl } : { uri: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4" }}
              ...
              onLoadStart={() => {
                console.log('🎬 Video load started for stream:', item.id, 'URL:', videoUrl);
                if (!paused && index === viewableIndex) setLoading(true);
              }}
              onLoad={(data) => {
                console.log('📊 Video load data for stream:', item.id, data);
              }}
              onReadyForDisplay={() => {
              ...
              onError={(error) => {
                ...
                const attempts = videoLoadAttempts[item.id] || 0;
                console.log('🔄 Video load attempt:', attempts + 1, 'for stream:', item.id);
                
                if (attempts < 2) {
                  // Retry loading the video
                  setVideoLoadAttempts(prev => ({ ...prev, [item.id]: attempts + 1 }));
                  setVideoError(prev => ({ ...prev, [item.id]: false }));
                } else {
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#000' }}>
              <Text style={{ color: '#fff', textAlign: 'center', marginTop: 40, fontSize: 16 }}>
                {videoError[item.id] ? 'Video failed to load' : 'No video URL available for this stream.'}
              </Text>
              ...
              {videoError[item.id] && (
                <TouchableOpacity 
                  ...
                  onPress={() => {
                    setVideoError(prev => ({ ...prev, [item.id]: false }));
                    setVideoLoadAttempts(prev => ({ ...prev, [item.id]: 0 }));
                  }}
                >
                  <Text style={{ color: '#fff', fontSize: 16 }}>Retry Video</Text>
                </TouchableOpacity>
              )}
            </View>
          )}
          ...
          <View style={styles.infoRowFull}>
            <View style={styles.leftInfoGroupFull}>
              ...
              <View style={styles.categoryRowLive}>
                ...
                <Text style={styles.categoryTextLive}>{item.category || "Gaming"}</Text>
              </View>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        >
          <View style={styles.noStreamsContainer}>
            ...
            <Text style={styles.noStreamsSubtitle}>
              {refreshing ? "Checking for new streams..." : "Come back later to see live streams!"}
            </Text>
            {refreshing && (
              <ActivityIndicator size="small" color="#006eff" style={{ marginTop: 16 }} />
            )}
          </View>
        </ScrollView>
      </SafeAreaView>
